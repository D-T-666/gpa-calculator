...
export function grade_group_points(
  user_data: { [item_name: string]: number | undefined },
  group_data: GradeGroup,
  max: boolean = false,
): number {
  return Object.entries(user_data)
    .map(
      ([key, value]) => value || (max ? group_data.items[key].max_points : 0),
    )
    .sort((a, b) => a - b)
...
export function course_points(
  user_course: UserSyllabusData,
  syllabus: Syllabus,
  max: boolean = false,
): number {
  let points = 0;
  for (let group_name of Object.keys(user_course)) {
    points += grade_group_points(
      user_course[group_name],
      syllabus[group_name],
      max,
    );
  }
...
export function course_GP(
  user_course: UserSyllabusData,
  course_curruculum: Syllabus,
  max: boolean = false,
): number {
  return points_to_GP(course_points(user_course, course_curruculum, max));
}

function semester_total_credits(
  user_semester: UserSemesterData,
  courses: CourseDict,
): number {
  let res = 0;
...
export function semester_GPA(
  ...
  for (let course_name of Object.keys(user_semester)) {
    if (user_semester[course_name].mode === "total") {
      if (
        !user_semester[course_name].total &&
        user_semester[course_name].total !== 0
      ) {
        acc += max ? courses[course_name].credits * points_to_GP(100) : 0;
      } else {
        acc +=
          courses[course_name].credits *
          points_to_GP(user_semester[course_name].total!);
      }
    } else {
      acc +=
        courses[course_name].credits *
        points_to_GP(
          course_points(
            user_semester[course_name].syllabus!,
            courses[course_name].syllabus!,
            max,
          ),
        );
    }
...
export function total_GPA(
  ...
  for (let [sem, sem_data] of Object.entries(user_data.semesters)) {
    acc +=
      semester_GPA(sem_data, courses, max) *
      semester_total_credits(sem_data, courses);
...
export function courses_taken_in_semester(
  user_semester: UserSemesterData,
): string[] {
  let taken: string[] = [];
  for (let course of Object.keys(user_semester)) {
    taken.push(course);
  }
  return taken;
}

export function courses_taken_before_semester(user_data: UserData, semester: number): string[] {
  let taken: string[] = [];
  for (let sem = 0; sem < semester; sem++) {
    taken = taken.concat(courses_taken_in_semester(user_data.semesters[sem]));
  }
  return taken;
}

export function coruses_taken(user_data: UserData): string[] {
  let taken: string[] = [];
  for (let sem_data of Object.values(user_data.semesters)) {
    taken = taken.concat(courses_taken_in_semester(sem_data));
  }
  return taken;
}

export function courses_completed(
  user_data: UserData,
  courses: CourseDict,
): string[] {
  let completed = [];
...
export function available_courses(
  user_data: UserData,
  semester: number,
  courses: CourseDict,
): string[] {
  let not_eligible = courses_taken_in_semester(user_data.semesters[semester]);
  let completed = courses_completed(user_data, courses);

  console.log("not eligible", not_eligible)

  let available_credits =
    semester % 2 === 0
      ? 36 - semester_total_credits(user_data.semesters[semester], courses)
      : 66 -
        semester_total_credits(user_data.semesters[semester], courses) -
        semester_total_credits(user_data.semesters[semester - 1], courses);

  let available = [];

  for (let [course, course_data] of Object.entries(courses)) {
    console.log(course);
    if ((course_data.parity === "fall") !== (semester % 2 === 0)) continue;

    if (course_data.min_semester > semester) continue;

    if (course_data.credits > available_credits) continue;
